...
<div class="row">
  <div class="col-md-12">
    <div class="card">
      <div class="card-header">
        ...
        <!-- search bar -->
        <form action="{{route('search_keyword_types')}}" class="form-wrapper">
          <div class="row">
            <!-- search bar -->
      ...
      <div class="card-body">
        <div class="col-md-12" style="overflow-x:auto;">
          <table id="example1" class="table text-nowrap  table-bordered table-striped dataTable dtr-inline table-sm" role="grid" aria-describedby="example1_info">
            ...
            <tbody>
              @if(count($keywordtypes) > 0)
              @foreach($keywordtypes as $keywordtype)
              <tr role="row" class="odd">
                <td>{{$keywordtype->id ? $keywordtype->id : NULL}}</td>
                <td>{{$keywordtype->keyword_name ? $keywordtype->keyword_name : NULL}}</td>
                <td>{{$keywordtype->created_by ? return_user_name($keywordtype->created_by) : NULL}}</td>
                <td>{{$keywordtype->modified_by ? return_user_name($keywordtype->modified_by) : NULL}}</td>
                <td width="100">
                  <!-- Detail -->
                  <a href="#" class="detailButton" data-id="{{$keywordtype->id}}">
                    <i class="fas fa-eye green ml-1"></i>
                  </a>
                  @if(auth()->user()->type == 'Manager' || auth()->user()->type == 'Admin' || auth()->user()->id == $keywordtype->created_by)
                  <!-- Edit -->
                  <a href="#" class="editButton" data-id="{{$keywordtype->id}}">
                    <i class="fas fa-edit blue ml-1"></i>
                  </a>
                  <!-- Delete -->
                  <a href="#" class="deleteButton" data-id="{{$keywordtype->id}}">
        ...
        <div class="card-footer">
          @if(count($keywordtypes) > 0)
          {{$keywordtypes->appends(request()->except('page'))->links('pagination::bootstrap-4')}}
          @endif
        </div>
      </div>
    </div>
  ...
  <div class="modal fade" id="addLeadModal" tabindex="-1" role="dialog" aria-labelledby="addLeadModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" style="max-width: 64rem;">
      <div class="modal-content">
        ...
        <form method="POST" action="{{route('keywordtype.store')}}" enctype="multipart/form-data">
          @include('admin.keyword_type.keywordtype_master')
  ...
  <div class="modal fade" id="editLeadModal" tabindex="-1" role="dialog" aria-labelledby="editLeadModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" style="max-width: 64rem;">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="editLeadModalLabel">Edit Keyword</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <form id="editForm" method="POST" action="{{route('keywordtype.update', 1)}}" enctype="multipart/form-data">
          <!-- hidden input -->
          @method('PUT')
  ...
  <div class="modal fade" id="viewLeadModal" tabindex="-1" role="dialog" aria-labelledby="addLeadModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">Keyword_Type Detail</h5>
          ...
          <div class="col-md-12">
            <table class="table table-bordered table-striped table-sm">
              <tbody id="table_row_wrapper">
                <!-- website_keyword_1 -->
                <tr role="row" class="odd">
                  <th class="">keyword_Type</th>
                  <td class="keyword_type"></td>
                </tr>
              </tbody>
            </table>
          </div>
  ...
  <div class="modal fade" id="deleteLeadModal" tabindex="-1" role="dialog" aria-labelledby="deleteLeadModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="deleteLeadModalLabel">Delete Keyword_Type</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <form id="deleteForm" method="POST" action="{{route('keywordtype.destroy', 1)}}">
          <!-- hidden input -->
          @method('DELETE')
          @csrf
  ...
  var element = $('li a[href*="keywordtype"]');
  element.parent().parent().parent().addClass('menu-open');
  element.addClass('active');

  // fetch keywordtype
  function fetch_keywordtype(id){
      // fetch keywordtype
      $.ajax({
        url: "<?php echo(route('keywordtype.show', 1)); ?>",
        type: 'GET',
        async: false,
        data: {id: id},
        dataType: 'JSON',
  ...
  $('.editButton').on('click', function(){
    var id = $(this).data('id');
    fetch_keywordtype(id);
    // console.log(keywordtype);
    $('#editForm #hidden').val(id);
    $('#editForm .keyword_type').val((keywordtype.keyword_name) ? (keywordtype.keyword_name) : '');
    $('#editForm .created_by').val((keywordtype.created_by) ? (keywordtype.created_by) : '');
    $('#editForm .modified_by').val((keywordtype.modified_by) ? (keywordtype.modified_by) : '');
  ...
  $('.detailButton').on('click', function(){
    ...
    $('#viewLeadModal .keyword_type').html(keywordtype.keyword_name);
  ...
  $('.deleteButton').on('click', function(){
    var id = $(this).data('id');
    $('#deleteForm').attr('action', "{{route('keywordtype.destroy', 1)}}");
    $('#deleteForm .hidden').val(id);
    $('#deleteLeadModalLabel').text('Delete Keyword Type ?');
